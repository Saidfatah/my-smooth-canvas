...
const ComposingPlayGround = ({
  ...
  const _shapesLocal = useRef([
    ...shapes.map((shape: shape) => ({ ...shape, isDragging: false }))
  ]);
  const canvasContext = useRef<CanvasRenderingContext2D | null>();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const selectedShapeIndex = useRef(-1);
  const selectedShapePreviousPositionState = useRef<shapePose>({ x: 0, y: 0 });
  ...
  const onMouseUp = useCallback(
    (e: MouseEvent) => {
      ...
      if (targetShape) {
        ...
        const xDiff =
          selectedShapePreviousPositionState.current.x - targetShape.x;
        const yDiff =
          selectedShapePreviousPositionState.current.y - targetShape.y;
        // we only want to track a new keyframe only when when the xDiff is greater than 5 pixels
        // or yDiff is greater than 5 pixels
        // otherwise we just can reset the psotions correspondignly
        if (xDiff > MINIMUM_DISTANCE_DIFF || yDiff > MINIMUM_DISTANCE_DIFF) {
          addNewKeyframe({
          ...
          updateShape({
            id: targetShape.id,
            position: { x: targetShape.x, y: targetShape.y }
          });
          toggleSelectShape({
            id: targetShape.id,
            value: !targetShape.isSelected
          });
        }
  ...
  const onMouseMove = useCallback(
    (ctx: CanvasRenderingContext2D | null, offsetX: number, offsetY: number) =>
      (e: MouseEvent) => {
        if (currentCanvasMode !== CANVAS_MODES.COMPOSING || ctx === null)
          return;
  ...
  useEffect(() => {
    if (canvasRef.current && canvasContext.current) {
      // console.log(shapes)
      _shapesLocal.current = [
        ...shapes.map((shape: any) => ({ ...shape, isDragging: false }))
      ];
...
const mapDispatch = (dispatch: Dispatch) => ({
  ...
  updateShape: dispatch.timeline.updateShape
});
